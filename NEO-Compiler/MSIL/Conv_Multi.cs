...
namespace Neo.Compiler.MSIL
{
    ...
    public partial class ModuleConverter
    {
        private void _ConvertStLoc(ILMethod method, OpCode src, NeoMethod to, int pos)
        {
            ...
            //_Convert1by1(Lux.VM.OpCode.CLONESTRUCTONLY, src, to);
            ////push d
            //var c = _Convert1by1(Lux.VM.OpCode.DEPTH, null, to);
            //if (c.debugcode == null)
            //{
            ...
            ////push olddepth
            //_Convert1by1(Lux.VM.OpCode.FROMALTSTACK, null, to);
            //_Convert1by1(Lux.VM.OpCode.DUP, null, to);
            //_Convert1by1(Lux.VM.OpCode.TOALTSTACK, null, to);
            ////(d-n-1)-olddepth
            //_Convert1by1(Lux.VM.OpCode.SUB, null, to);
            ...
            ////swap d-n-1 and top
            //_Convert1by1(Lux.VM.OpCode.XSWAP, null, to);
            ////drop top
            //_Convert1by1(Lux.VM.OpCode.DROP, null, to);
        ...
        private void _ConvertLdArg(ILMethod method, OpCode src, NeoMethod to, int pos)
        {
            ...
            ////push d
            //var c = _Convert1by1(Lux.VM.OpCode.DEPTH, src, to);
            //if (c.debugcode == null)
            //{
            ...
            ////push n
            //_ConvertPush(pos, null, to);//翻转取参数顺序
            ////_Convert1by1(Lux.VM.OpCode.PUSHDATA1, null, to, int2Pushdata1bytes(to.paramtypes.Count - 1 - pos));
            ////d+n
            //_Convert1by1(Lux.VM.OpCode.ADD, null, to);
		...
		*/
		...
		public bool IsMixAttribute(Mono.Cecil.MethodDefinition defs, out Lux.VM.OpCode[] opcodes, out string[] opdata)
		{
			// ============================================
			...
			opcodes = new Lux.VM.OpCode[count_attrs];
			...
			foreach (var attr in defs.CustomAttributes)
			{
				if (attr.AttributeType.Name == "OpCodeAttribute")
				{
					opcodes[i] = (Lux.VM.OpCode)attr.ConstructorArguments[0].Value;
				...
				else if (attr.AttributeType.Name == "SyscallAttribute")
				{
					...
					opcodes[i] = Lux.VM.OpCode.SYSCALL;
					opdata[i] = val;
	...
	*/
		...
		private int _ConvertCall(OpCode src, NeoMethod to)
		{
			...
			int callpcount = 0;
			byte[] callhash = null;
			//VM.OpCode callcode = Lux.VM.OpCode.NOP;
			Lux.VM.OpCode[] callcodes = null;
			...
			else if (IsMixAttribute(defs, out callcodes, out calldata))
			{
				...
				if (callcodes.Length == 1 && callcodes[0] != Lux.VM.OpCode.SYSCALL)
				{
					calltype = 2;
				}
			...
			{//maybe a syscall // or other
				...
				else if (src.tokenMethod.Contains("::op_Inequality("))
				{
					...
					{
						_Convert1by1(Lux.VM.OpCode.EQUAL, src, to);
						_Convert1by1(Lux.VM.OpCode.NOT, null, to);
					}
					////各类!=指令
					////有可能有一些会特殊处理，故还保留独立判断
			...
			{//翻转参数顺序
				...
				//if ((calltype == 3) || ((calltype == 7) && (callcodes[0] == Lux.VM.OpCode.SYSCALL)))
				//    pcount++;
			...
			if (calltype == 1)
			{
				if (this.outModule.option.useNep8)
				{
					...
					var c = _Convert1by1(Lux.VM.OpCode.CALL_I, null, to, new byte[] { _rvcount, _pcount, 0, 0 });
					c.needfixfunc = true;
            ...
            */
			else if (calltype == 7)
			{
				for (var j = 0; j < callcodes.Length; j++)
				{
					...
					if (callcodes[j] == Lux.VM.OpCode.SYSCALL)
					{
						//if(isHex)
						//{
			...
			else if (calltype == 4)
			{
				if (this.outModule.option.useNep8)
				{
					...
					else
					{
						...
						_Convert1by1(Lux.VM.OpCode.CALL_E, null, to, bytes);
			...
			else if (calltype == 6)
			{
				...
				if (this.outModule.option.useNep8)
				{
					...
					//    );
					_Convert1by1(Lux.VM.OpCode.CALL_ED, null, to, new byte[] { _rvcount, _pcount });
				}
				else
				{
		...
		private int _ConvertNewArr(ILMethod method, OpCode src, NeoMethod to)
		{
			...
			{
				_Convert1by1(Lux.VM.OpCode.NEWARRAY, src, to);
			...
			else // (type == "System.Byte") || (type == "System.SByte")
			{
				...
				//we need a number
				if (code.code > Lux.VM.OpCode.PUSH16)
				{
        ...
        private int _ConvertInitObj(OpCode src, NeoMethod to)
        {
            ...
            //_Convert1by1(Lux.VM.OpCode.SWAP, null, to);//replace n to top

            ////push d
            //_Convert1by1(Lux.VM.OpCode.DEPTH, null, to);

            //_Convert1by1(Lux.VM.OpCode.DEC, null, to);//d 多了一位，剪掉
            //_Convert1by1(Lux.VM.OpCode.SWAP, null, to);//把n拿上來
            ////push n
                ...
                if (m.IsConstructor && m.HasCustomAttributes)
                {
                    foreach (var attr in m.CustomAttributes)
                    {
                        if (attr.AttributeType.Name == "OpCodeAttribute")
                        {
							...
							var opcode = (Lux.VM.OpCode)attr.ConstructorArguments[0].Value;
							var opdata = Helper.OpDataToBytes((string)attr.ConstructorArguments[1].Value);
							Lux.VM.OpCode v = (Lux.VM.OpCode)opcode;
        ...
        private int _ConvertStfld(ILMethod method, OpCode src, NeoMethod to)
        {
            ...
            _Convert1by1(Lux.VM.OpCode.SWAP, null, to);//把item 拿上來

            _Convert1by1(Lux.VM.OpCode.SETITEM, null, to);//修改值 //item //index //array
            return 0;
        }
        ...
        private int _ConvertLdfld(OpCode src, NeoMethod to)
        {
            ...
            _Convert1by1(Lux.VM.OpCode.PICKITEM, null, to);//修改值
